...
Uso tipico (dentro de preprocess_exam, apos filtragem por FILTER_PARAMS):
    ...
    if ecg_present and emg_present:
        ...
        emg_gated, n_gated = gate_emg_by_ecg(emg_uv, r_peaks, fs, window_s=0.05)
"""
...
def apply_ecg_gating_to_raw(
    ...
    *,
    ...
    window_s : float
        Semi-largura da janela de gating em segundos (ver gate_emg_by_ecg).
    verbose : bool
    ...
    emg_gated, n_gated = gate_emg_by_ecg(emg_data, r_peaks, fs, window_s=window_s)
...
def gate_emg_by_ecg(
    ...
    *,
    ...
    """
    Remove a contaminacao de QRS do EMG por interpolacao linear numa janela
    estreita centrada em cada pico-R (gating).

    Para cada pico-R em `r_peaks`, substitui as amostras de `emg` no
    intervalo [r_peak - window_s, r_peak + window_s] por uma interpolacao
    linear entre os dois valores nas bordas da janela -- em vez de zerar
    (que criaria uma queda espuria e artificial no envelope RMS), a
    interpolacao preserva o nivel local do sinal sem introduzir a
    ...
    fs : float
        Taxa de amostragem em Hz.
    window_s : float
        Semi-largura da janela de gating em segundos (default 0.05s = 50ms
        de cada lado do pico-R, i.e. 100ms de janela total -- cobre a
        duracao tipica do complexo QRS, ~80-120ms).
    ...
    half_w = max(1, int(round(window_s * fs)))
    gated_mask = np.zeros(n, dtype=bool)

    for peak in r_peaks:
        i0 = max(0, int(peak) - half_w)
        i1 = min(n - 1, int(peak) + half_w)
        if i1 <= i0:
            continue
        left_val = emg[i0]
        right_val = emg[i1]
