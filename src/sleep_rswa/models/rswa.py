...
def _select_stage_context(stage_probs: torch.Tensor | None, cfg: ModelConfig) -> torch.Tensor | None:
...
class RSWADetectionNet(nn.Module):
    def __init__(self, config=None, use_se=True, *, stage_conditioning: bool | None = None):
        ...
        self.stage_context_dim = _stage_context_dim(cfg)
        self.stage_fusion = (
            nn.Sequential(
                nn.Linear(
    ...
    def forward(self, emg_center, mask=None, stage_probs=None):
        ...
        stage_context = _select_stage_context(stage_probs, self.cfg)
        if self.stage_fusion is not None and stage_context is not None:
            if stage_context.shape[:2] != z.shape[:2]:
                raise ValueError(
                    "stage_probs deve alinhar com (B,T) das features RSWA; "
                    f"recebeu {tuple(stage_context.shape)} para features "
                    f"{tuple(z.shape)}."
                )
            if stage_context.shape[-1] != self.stage_context_dim:
                raise ValueError(
            ...
            z = self.stage_fusion(
                torch.cat([z, stage_context.to(z.dtype)], dim=-1)
            )
