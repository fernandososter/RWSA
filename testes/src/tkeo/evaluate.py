"""
Avaliacao intervalar do metodo TKEO (vs. RMS) em exames reais -- MESMA
metodologia usada em testes/src/limiar/evaluate.py apos a revisao do
usuario (ver docstring daquele arquivo):
...
4. Executa o detector (RMS ou TKEO) nesse recorte, sem varrer o exame
   inteiro -- a baseline e o limiar sao calculados apenas dentro do
   recorte do intervalo anotado.
5. Classificacao por duracao + amplitude identica a
   testes/src/limiar/threshold_rule.py: fasico 0.1-5.0s, any (ambigua)
   5.0-15.0s (exclusive-exclusive), tonico >=15.0s, todos exigindo
   score (pico do sinal de energia / baseline local) >= 2.0.

Isolado: usa apenas tkeo_rule.py (deste mesmo diretorio) + torch/numpy/
pandas. Nao importa nada de classifier/, src/sleep_rswa/ nem de
testes/src/limiar/ (reimplementado aqui para manter a pasta autocontida).

Saidas (em testes/src/tkeo/results/), MESMO FORMATO de
testes/src/limiar/results/ para permitir comparacao direta entre metodos:
...
from tkeo_rule import detect_events_rms, detect_events_tkeo
...
DETECT_FNS = {
    "rms": lambda emg, fs: detect_events_rms(emg, fs=fs, apply_merge_gaps=True),
    "tkeo": lambda emg, fs: detect_events_tkeo(emg, fs=fs, apply_merge_gaps=True),
}
...
def main():
    ...
    print("=== ANALISE POR INTERVALO ANOTADO (TKEO vs RMS) ===")
